...
printf 'Query app: %s\n' "$(azd env get-value QUERY_CONTAINER_APP_NAME 2>/dev/null || printf '<run azd up first>')"
printf 'Internal catalog app: %s\n' "$(azd env get-value CATALOG_CONTAINER_APP_NAME 2>/dev/null || printf '<run azd up first>')"
